...
def profile_memory_and_latency(model_path, device="cuda:0"):
    ...
    # Profile without Hetero Cache
    print("\n▶ Profiling with Native Cache...")
    showcase_memory_probe_native = ShowcaseMemoryProbe(device, "Native", base_mem=None)  # adjusted probe arguments
    try:
        with torch.inference_mode():
            start_time = time.perf_counter()
            outputs_native = model.generate(
                input_ids=input_ids, max_new_tokens=15,
                logits_processor=LogitsProcessorList([showcase_memory_probe_native])
            )
            showcase_memory_probe_native.calculate_latency(start_time)
        decode_peak_native = torch.cuda.max_memory_allocated(device) / 1024 ** 3
        resp_native = processor.batch_decode([out[len(input_ids[0]):] for out in outputs_native], skip_special_tokens=True)[0]
        display_profile_results(showcase_memory_probe_native, decode_peak_native, resp_native)
    except torch.OutOfMemoryError:
        print("   💥 Native OOM 崩溃！")
        display_profile_results(showcase_memory_probe_native, float('inf'), "OOM FAILED")

    # Profile with Hetero Cache
    print("\n▶ Profiling with Hetero Cache...")
    hetero_cache = HeteroTransientCache(sink_tokens=64, keep_tail=8192)
    showcase_memory_probe_hetero = ShowcaseMemoryProbe(device, "Hetero", base_mem=None, cache=hetero_cache)  # adjusted probe arguments
    try:
        with torch.inference_mode():
            start_time = time.perf_counter()
            outputs_hetero = model.generate(
                input_ids=input_ids, max_new_tokens=15, past_key_values=hetero_cache,
                logits_processor=LogitsProcessorList([showcase_memory_probe_hetero])
            )
            showcase_memory_probe_hetero.calculate_latency(start_time)
        decode_peak_hetero = torch.cuda.max_memory_allocated(device) / 1024 ** 3
        resp_hetero = processor.batch_decode([out[len(input_ids[0]):] for out in outputs_hetero], skip_special_tokens=True)[0]
        display_profile_results(showcase_memory_probe_hetero, decode_peak_hetero, resp_hetero)
    except Exception as e:
        print(f"   💥 Hetero 崩溃: {e}")
        display_profile_results(showcase_memory_probe_hetero, float('inf'), "CRASHED")
