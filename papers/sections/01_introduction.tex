...
\begin{itemize}
    ...
    \item \textbf{4-Bit Hierarchical Compression}: Unlike eviction-based methods that permanently discard tokens, we apply group-wise asymmetric INT4 quantization (48\% storage reduction via uint8-packed format, with 4-bit theoretical compression target) to offload overflow KV tensors from HBM to DRAM. This preserves the ability to reconstruct any portion of the sequence when needed, enabling a true two-tier storage hierarchy.
